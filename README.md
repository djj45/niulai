...
| **搜索结果的滑轨 / 日历** | 搜索结果也是一条连续时间线：先显示最新 1000 条命中，往上滚接着翻更早的，命中再多也能全部看到。右侧滑轨换成**命中的滑轨**（绿色，每天一段、长度 = 当天命中数），日期弹出的是**命中日历**（绿色，一格一天，可切「全部命中 / 老师的命中」）；‹ › 在有命中的日子之间跳；拖滑轨、点日历格子、滚轮都只在命中里定位，不会退出搜索。计数显示 `命中 N 条 · M 天 · 本日 K` |
