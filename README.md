...
| **老师新消息提醒** | 顶栏「🔕/🔔 提醒」：可分别开「系统通知」（浏览器通知 = Windows / macOS 原生通知中心）和「提示音」（页面现场合成的「叮-咚」，不用音频文件）；「正在看这个页面时也弹通知」默认关（正看着就只响）。只提醒实时推来的老师消息（补同步的历史、10 分钟前的重连补推不提醒）；老师连发 1.5 秒内只响一次；页面在后台时标签页标题显示未读数；点通知跳到那条消息。设置存在本机浏览器里。注意：页面要开着；浏览器要求先点过页面才能出声（刷新后点任意处即可）；收不到先查系统通知设置里是否允许浏览器、勿扰模式是否关着 |
...
**Q：勾了「系统通知」却收不到？**
A：通知是浏览器发的，系统那边也要放行：macOS「系统设置 → 通知 → 你的浏览器（Chrome / Edge / Safari）」打开允许；
Windows「设置 → 系统 → 通知」里允许浏览器，并关掉专注助手 / 勿扰。浏览器这边若点过「阻止」，
在地址栏左边的站点图标里把「通知」改回允许。页面标签要开着（最小化没关系）。

**Q：提示音有时不响？**
A：浏览器规定页面没被点过就不能出声。刷新页面后随便点一下任意位置，之后就能正常响；提醒面板里会提示「提示音待激活」。
